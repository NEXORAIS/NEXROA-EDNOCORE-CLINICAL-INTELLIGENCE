...
export function calcFIB4(p) {
  if (v(p.plaquetas) <= 0 || v(p.alt) <= 0) return 0;
...
export function calcEGFR(p) {
  ...
  if (cr <= 0 || edad <= 0 || !p.sexo) {
    return v(p.tfg) || 0; // fallback: valor manual del formulario
  }
...
/**
 * Clasificación de riesgo hepático por FIB-4 (versión simple, corte plano 1.3).
 *
 * NOTA (auditoría 8-ago-2026): esta versión YA NO se usa en ningún render —
 * render.js's renderGlobalStrat() fue migrada a individualization.js ->
 * classifyFIB4(fib4, edad, vctLsm), que ajusta el corte a 2.0 en ≥65 años y
 * pide VCTE-LSM antes de confirmar riesgo alto (consenso MASLD 2026). Usar
 * ESTA versión en cualquier tarjeta visible reintroduciría el bug ya
 * corregido: mismo paciente ≥65 años mostrando "RIESGO ELEVADO" aquí y
 * "Riesgo Bajo" en la tarjeta de Individualización de al lado. Se deja
 * definida (código muerto documentado, mismo criterio que classifyGlobalRisk
 * más abajo) por si algún consumidor futuro la necesita para otro propósito,
 * pero NO debe usarse para clasificar riesgo de fibrosis frente al usuario.
 */
export function classifyFIB4(fib4) {
  return fib4 > 1.3
    ? { label: "RIESGO ELEVADO", isHigh: true }
    : { label: "RIESGO BAJO", isHigh: false };
}
...
/**
 * Riesgo global simplificado. NOTA (auditoría 8-ago-2026): esta función no
 * se llama desde ningún lado del proyecto actual (se reemplazó por el panel
 * de Individualización + Estratificación Global) — es código muerto. Tenía
 * el MISMO bug de `p.tfg` fantasma que ya se había corregido en
 * `classifyLipidRisk`; como nunca se ejecuta, nunca causó un problema visible,
 * pero se corrige aquí también para que si en el futuro se reconecta, no
 * reintroduzca el bug de "todo paciente cae en ALTO siempre".
 */
export function classifyGlobalRisk(p) {
  const a1c = getA1cEfectiva(p).value;
  const egfr = calcEGFR(p);
  let risk = "BAJO";
  if ((egfr > 0 && egfr < 60) || a1c > 8) risk = "MODERADO";
  if ((p.comorbilidades && p.comorbilidades.length > 0) || (egfr > 0 && egfr < 45)) risk = "ALTO";
  return risk;
}
...
export function buildAntidiabeticPlan(p) {
  ...
  } else if (!metInPlan && (flags.egfr === 0 || flags.egfr >= 30)) {
...
export function buildTreatmentPlan(p) {
  ...
  if (redFlags.activo) {
    return { items: [], dmNote: null, flags: {}, interactionWarnings: [], redFlags };
  }
  ...
  return { items: out, dmNote: dm.note, flags: dm.flags, interactionWarnings, redFlags };
}
