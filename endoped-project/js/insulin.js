...
export function computeBasalInsulinDose(p) {
  ...
  // CORRECCIÓN DE GOBERNANZA (16-ago-2026, Dr. Ortega — hallazgo del
  // catálogo de gobernanza clínica): el orden de estos dos `if` importaba
  // más de lo que parecía. Antes, `basalInsuficiente` (ayuno/nocturna
  // elevados) se revisaba PRIMERO — así que un caso mixto (ayuno elevado
  // en el mismo paciente cuya nocturna está en hipoglucemia) devolvía
  // "escalar la dosis basal" sin llegar nunca a revisar `basalExcesiva`.
  // Escalar una basal que YA está causando hipoglucemia nocturna es
  // exactamente el error de dosificación que este sistema existe para
  // evitar. `computeMonitoreo` (arriba, línea ~125) YA prioriza
  // `basalExcesiva` primero en su propio mensaje de interpretación — este
  // orden solo alinea la recomendación de DOSIS con el criterio de
  // prioridad que el mensaje de monitoreo ya usaba.
  if (basalExcesiva) {
    return { estado: "establecida_riesgo_hipoglucemia", fuente: "Ayuno/nocturna bajo meta (o en rango de hipoglucemia) -> considerar REDUCIR la dosis basal actual antes de cualquier otro ajuste, incluso si el ayuno en otro punto está elevado — no escalar sobre una hipoglucemia ya presente." };
  }
