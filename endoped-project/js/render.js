...
export function renderGlobalStrat(p) {
  ...
  // ACTUALIZACIÓN (16-ago-2026): `calc.classifyFIB4` (la versión sin ajustar
  // por edad, mencionada arriba) YA NO EXISTE — se eliminó de calculations.js
  // por ser código muerto peligroso (ver catálogo de gobernanza, categoría I).
  // Este párrafo se conserva como registro histórico de por qué la tarjeta
  // duplicada se quitó de aquí, no como advertencia vigente.
...
function renderIndividualizationPanel(p) {
  ...
  renderIndividualizationCard("individualizationCards", [
    ...
    // Corrección 16-ago-2026: getCardiovascularSummary ya hace todo el árbol
    // de decisión (ASCVD establecida / fuera de rango PREVENT / sin capturar)
    // y lee p.preventAscvd10 (el campo real que sí existe en state.js — antes
    // se llamaba aquí classifyASCVD(p.ascvdRiskPct), un campo nunca asignado,
    // por lo que esta tarjeta SIEMPRE mostraba "Sin capturar").
    { icon: "activity", title: "Riesgo ASCVD (PREVENT)", result: getCardiovascularSummary(p, flags) },
...
/**
 * Aviso NO bloqueante de "dato de seguridad no verificado" (16-ago-2026, Dr.
 * Ortega — capa central de gobernanza clínica). `s.caution` lo adjunta
 * calculations.js -> annotateRenalCaution cuando el fármaco tiene un mínimo
 * renal conocido pero el eGFR del paciente no se pudo calcular (falta
 * creatinina). A diferencia de una contraindicación real, esto NUNCA quita
 * el fármaco de la tarjeta — solo avisa, con el mismo espíritu que las
 * zonas límite de boundary-alerts.js: el sistema no decide por el médico,
 * le muestra lo que no pudo confirmar.
 */
function buildCautionHTML(s) {
  if (!s.caution) return "";
  return `<p class="mt-1.5 flex items-start gap-1 text-[10px] font-bold text-amber-600 dark:text-amber-400">
    <i data-lucide="shield-alert" class="w-3 h-3 shrink-0 mt-0.5"></i>
    <span>${s.caution}</span>
  </p>`;
}
...
export function renderManagement(p) {
  ...
  if (container) {
    ...
    const { items, dmNote, interactionWarnings, redFlags, reviewCautions } = calc.buildTreatmentPlan(p);
    ...
    // Secciones no revisadas (Comorbilidades/Antecedentes/Medicación Actual)
    // — ver buildReviewCautions en calculations.js. No bloquea nada, solo
    // avisa y ofrece un salto directo a la sección (jumpToIngresoSection,
    // navigation.js) — mismo botón "ir a los datos faltantes" pedido por el
    // Dr. Ortega para la Brújula de Precisión.
    if (reviewCautions && reviewCautions.length > 0) {
      container.innerHTML += reviewCautions.map((c) => `
        <div class="p-3 mb-3 rounded-xl border bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 text-xs flex items-start justify-between gap-3">
          <p class="flex items-start gap-2"><i data-lucide="list-checks" class="w-4 h-4 shrink-0 mt-0.5"></i><span>${esc(c.texto)}</span></p>
          <button type="button" onclick="jumpToIngresoSection(${c.section})" class="shrink-0 px-2.5 py-1 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-[10px] font-bold uppercase tracking-wide whitespace-nowrap">Revisar</button>
        </div>`).join("");
    }
