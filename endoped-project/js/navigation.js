...
/**
 * Salta directamente a una sección del Ingreso Clínico y la ABRE (nunca la
 * cierra si ya estaba abierta — a diferencia de toggleSection, que alterna).
 * Usado por los avisos de "sección no revisada" (calculations.js
 * reviewCautions) y por la Brújula de Precisión (16-ago-2026, a petición
 * del Dr. Ortega: "un botón que redirija a esos datos faltantes"). Hace
 * scroll suave hasta el botón de la sección para que el título quede
 * visible arriba, no solo el contenido.
 */
export function jumpToIngresoSection(id) {
  showIngreso();
  for (let i = 1; i <= 7; i++) {
    const body = document.getElementById(`body-section-${i}`);
    const icon = document.getElementById(`icon-section-${i}`);
    if (!body) continue;
    const shouldBeOpen = i === id;
    const isOpen = body.classList.contains("open");
    if (shouldBeOpen === isOpen) continue;
    body.classList.toggle("open", shouldBeOpen);
    body.style.maxHeight = shouldBeOpen ? body.scrollHeight + "px" : null;
    if (icon) icon.style.transform = shouldBeOpen ? "rotate(180deg)" : "rotate(0deg)";
    if (shouldBeOpen) body.dataset.visited = "1";
  }
  renderIngresoProgress();
  requestAnimationFrame(() => {
    document.getElementById(`section-toggle-${id}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  });
}
