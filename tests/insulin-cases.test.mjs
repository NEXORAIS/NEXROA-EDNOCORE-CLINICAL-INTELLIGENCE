...
// I23: GOBERNANZA (16-ago-2026) — caso mixto: ayuno elevado (>130) Y
// nocturna en hipoglucemia (<70) simultáneamente. Antes, el orden de los
// `if` en computeBasalInsulinDose revisaba `basalInsuficiente` primero y
// devolvía "escalar hacia 0.5 UI/kg/día" sin llegar nunca a ver la
// hipoglucemia nocturna — exactamente el error que el propio mensaje de
// computeMonitoreo ya advertía (línea ~125: basalExcesiva se prioriza en el
// texto de interpretación, pero la función de dosis no lo respetaba).
{
  const pMixto = { ...conGlargina, peso: 80, edad: 50, saludStatus: "sano", hba1c: 7.5, glucosa: 150, glucosaNocturna: 60 };
  const m = computeMonitoreo(pMixto);
  check("I23", "Caso mixto: ayuno 150 (elevado) + nocturna 60 (hipoglucemia) -> monitoreo detecta AMBAS señales",
    m.basalInsuficiente === true && m.basalExcesiva === true, JSON.stringify(m));
  const b = computeBasalInsulinDose(pMixto);
  check("I23", "Caso mixto -> la DOSIS recomienda REDUCIR (no escalar) — hipoglucemia manda sobre ayuno elevado",
    b.estado === "establecida_riesgo_hipoglucemia", JSON.stringify(b));
  check("I23", "Caso mixto -> NUNCA debe devolver 'establecida_no_en_metas' (que implicaría escalar sobre una hipoglucemia)",
    b.estado !== "establecida_no_en_metas", JSON.stringify(b));

  // Control: solo ayuno elevado, nocturna en meta -> sigue escalando como siempre (comportamiento previo intacto).
  const pSoloAyuno = { ...conGlargina, peso: 80, edad: 50, saludStatus: "sano", hba1c: 7.5, glucosa: 150, glucosaNocturna: 100 };
  const bSoloAyuno = computeBasalInsulinDose(pSoloAyuno);
  check("I23", "Control: solo ayuno elevado (nocturna en meta) -> SIGUE escalando (sin cambio de comportamiento)",
    bSoloAyuno.estado === "establecida_no_en_metas", JSON.stringify(bSoloAyuno));
}
