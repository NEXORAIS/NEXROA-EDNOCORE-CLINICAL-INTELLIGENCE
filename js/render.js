...
import { renderInsulinPanel } from "./insulin.js";
 ...
 * Antes vivían dispersas (LDL en EndoLypids, A1c en Individualización, sin
 * lugar consolidado para glucosa en ayuno/colesterol total/TG/TA) — esto no
 * agrega criterios nuevos, solo reutiliza los ya verificados en
 * individualization.js y calculations.js para que el médico los vea juntos.
 */
export function renderGoals(p) {
  ...
  const a1c = calc.getA1cEfectiva(p);
  setText("goalA1cActual", a1c.value || "--");
  setText("goalA1cMeta", goals.a1c);

  setText("goalAyunoActual", p.glucosa || "--");
  setText("goalAyunoMeta", goals.ayuno);

  setText("goalBPActual", p.tas && p.tad ? `${p.tas}/${p.tad}` : "--/--");
  setText("goalBPMeta", goals.bp);

  setText("goalColActual", p.col_total || "--");

  const { label: riesgoLDL, target: metaLDL } = calc.classifyLipidRisk(p);
  setText("goalLDLActual", p.ldl || "--");
  setText("goalLDLRiesgo", riesgoLDL);
  setText("goalLDLMeta", `<${metaLDL} mg/dL`);

  setText("goalTGActual", p.trigliceridos || "--");
  setText("goalTGMeta", TG_GOAL.label);
}
...
/** Posición (0-100%) en una escala logarítmica de 1 a 500 h, para el gauge de vida media. */
function vidaMediaPct(horas) {
  const clamped = Math.min(500, Math.max(1, horas));
  return ((Math.log10(clamped) - Math.log10(1)) / (Math.log10(500) - Math.log10(1))) * 100;
}
...
export function showPharmaDetail(id, cardEl) {
  ...
  // --- 4. Duración (gauge visual de vida media) ---
  const pct = vidaMediaPct(f.vidaMediaHoras || 1);
  const duracionHtml = f.vidaMediaHoras ? `
    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
      <div class="flex items-center justify-between mb-2">
        <span class="flex items-center gap-2 text-[11px] font-black text-indigo-500 uppercase"><i data-lucide="hourglass" class="w-3.5 h-3.5"></i> Duración (Vida Media)</span>
        <span class="font-data font-bold text-indigo-700 dark:text-indigo-300 text-sm">${f.vidaMediaLabel}</span>
      </div>
      <div class="relative h-2.5 w-full bg-gradient-to-r from-blue-200 via-emerald-200 to-purple-300 dark:from-blue-900/50 dark:via-emerald-900/50 dark:to-purple-900/50 rounded-full">
        <div class="absolute -top-1 w-2 h-4 bg-slate-800 dark:bg-white rounded shadow" style="left:calc(${pct}% - 4px)"></div>
      </div>
      <div class="flex justify-between text-[9px] font-bold text-slate-400 uppercase mt-1"><span>Horas</span><span>Días</span><span>Semanas</span></div>
    </div>` : "";
  ...
  panel.innerHTML = `<div class="w-full text-left space-y-4">
    ...
    <!-- 4b: Curva PK/PD 24h (Dr. Ortega, 10/11-ago-2026) -->
    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-[10px] font-bold text-slate-400">${archetypeLabel(f)}</span>
      </div>
