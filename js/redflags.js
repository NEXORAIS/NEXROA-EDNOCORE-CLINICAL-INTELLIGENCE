...
const RED_FLAG_DEFS = [
  ...
  {
    // VERIFICACIÓN DE GOBERNANZA (16-ago-2026, Dr. Ortega): releído contra
    // el PDF completo de la guía 2025 AHA/ACC/.../SGIM (Circulation.
    // 2025;152:e114–e218 — no existe edición "2026"). El texto define
    // "severe hypertension"/"hypertensive emergency" con ">180/120"
    // (estrictamente mayor), citado así 5 veces en el documento. Este
    // guardrail usa "≥180/120" a propósito — decisión explícita de
    // mantenerlo conservador, para que un paciente exactamente en 180/120
    // siga derivándose a evaluación urgente. Debe coincidir siempre con
    // calculations.js -> classifyBP e individualization.js -> classifyBP
    // (los 3 puntos usan "≥", ninguno usa ">").
    id: "CRISIS_HIPERTENSIVA",
    label: "Crisis hipertensiva",
    check: (p) => v(p.tas) >= 180 || v(p.tad) >= 120,
    detalle: (p) => `PA ${p.tas || "--"}/${p.tad || "--"} mmHg — umbral de Crisis Hipertensiva (ACC/AHA 2025, ≥180/120 mmHg por decisión conservadora del proyecto; el texto literal de la guía dice ">180/120").`,
  },
  {
    id: "EHH_RANGO",
