...
// VERIFICACIÓN DE GOBERNANZA (16-ago-2026, Dr. Ortega): cortes releídos
// contra el PDF completo de la guía (2025 AHA/ACC/.../SGIM, Circulation.
// 2025;152:e114–e218 — no existe edición "2026"). El umbral de "Crisis
// Hipertensiva" (≥180/120) usa "mayor o igual" a propósito, aunque el texto
// literal de la guía dice ">180/120" (estrictamente mayor) — decisión
// explícita de mantenerlo conservador (ver misma nota en calculations.js
// -> classifyBP y en redflags.js -> CRISIS_HIPERTENSIVA, los 3 puntos
 ...
 * La ecuación PREVENT en sí NO está implementada dentro del dashboard: el
 * médico captura el % ya calculado aparte (heart.org/prevent, MDCalc) en el
 * campo `preventAscvd10` del Ingreso Clínico; esta función solo decide CUÁL
 * score mostrar y cómo colorearlo.
 *
 * CORRECCIÓN (16-ago-2026, hallazgo de la auditoría de precisión de campos
 * a petición del Dr. Ortega): esta función y `classifyCKM` (Estadio 3, más
 * abajo) leían `p.ascvdRiskPct` — un nombre de campo que `buildPatientFromForm`
 * (state.js) NUNCA asignó. Solo existía `p.preventAscvd10` (mismo concepto
 * clínico: % de riesgo ASCVD a 10 años vía PREVENT, ya usado por
 * classifyLipidRisk en calculations.js). Resultado: esta tarjeta y la vía
 * "PREVENT≥20%" de CKM Estadio 3 SIEMPRE mostraban "Sin capturar", sin
 * importar que el médico sí hubiera registrado el dato. Corregido apuntando
 * ambas lecturas a `p.preventAscvd10` — una sola fuente de verdad, en vez de
 * introducir un segundo campo que habría que mantener sincronizado a mano
 * (mismo principio que la eliminación de classifyFIB4/classifyGlobalRisk
 * duplicados en calculations.js).
 */
export function getCardiovascularSummary(p, flags) {
  ...
  if (p?.preventAscvd10 === undefined || p?.preventAscvd10 === null || p?.preventAscvd10 === "") {
    return { valor: "--", label: "% PREVENT", zona: "gris", color: ZONES.gris, texto: "Sin capturar", detalle: "Ingresa el % de riesgo PREVENT-ASCVD a 10 años (calculado aparte); la ecuación aún no está implementada dentro del dashboard." };
  }
  return classifyASCVD(Number(p.preventAscvd10));
}
...
export function classifyCKM(p, flags, egfr, imc) {
  ...
  // Corrección 16-ago-2026: era patient.ascvdRiskPct (campo nunca asignado
  // por state.js, ver comentario de getCardiovascularSummary arriba).
  const prevent = num(patient.preventAscvd10);
